...
import { SchoolWhereInput } from "../school/SchoolWhereInput";
...
export class StudentController {
    ...
    if (invalidAttributes.length) {
      ...
      throw new common.ForbiddenException(
        `Updating the relationship: ${
          invalidAttributes[0]
        } of ${"Student"} is forbidden for roles: ${roles}`
      );
    }
  ...
  async findManySchools(
    ...
    const results = await this.service.findOne({ where: params }).schools({
      where: query,
      select: {
        createdAt: true,
        id: true,

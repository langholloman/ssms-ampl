...
import { StudentWhereInput } from "../student/StudentWhereInput";
...
export class ClassController {
  ...
  @swagger.ApiCreatedResponse({ type: Class })
  @swagger.ApiForbiddenResponse({ type: errors.ForbiddenException })
  async create(
    @common.Query() query: {},
    @common.Body() data: ClassCreateInput,
  ...
  @swagger.ApiOkResponse({ type: [Class] })
  ...
  async findOne(
    @common.Query() query: {},
    @common.Param() params: ClassWhereUniqueInput,
    @nestAccessControl.UserRoles() userRoles: string[]
  ): Promise<Class | null> {
    const permission = this.rolesBuilder.permission({
      role: userRoles,
  ...
  @common.UseInterceptors(nestMorgan.MorganInterceptor("combined"))
  @common.UseGuards(basicAuthGuard.BasicAuthGuard, nestAccessControl.ACGuard)
  @common.Get("/:id/instructor")
  @nestAccessControl.UseRoles({
    resource: "Class",
    action: "read",
    possession: "any",
  })
  async findManyInstructor(
    @common.Param() params: ClassWhereUniqueInput,
    ...
    const results = await this.service.findOne({ where: params }).instructor({
      where: query,
      select: {
        createdAt: true,
        emailAddress: true,
  ...
  async createInstructor(
    @common.Param() params: ClassWhereUniqueInput,
    @common.Body() body: ClassWhereUniqueInput[],
    @nestAccessControl.UserRoles() userRoles: string[]
  ): Promise<void> {
    const data = {
      instructor: {
    ...
    if (invalidAttributes.length) {
      ...
      throw new common.ForbiddenException(
        `Updating the relationship: ${
          invalidAttributes[0]
        } of ${"Class"} is forbidden for roles: ${roles}`
      );
    }
  ...
  async findManyStudent(
    ...
    const results = await this.service.findOne({ where: params }).student({
      where: query,
      select: {
        createdAt: true,
        firstName: true,
  ...
  async findManyUser(
    ...
    const results = await this.service.findOne({ where: params }).user({
      where: query,
      select: {
        createdAt: true,
        emailAddress: true,
